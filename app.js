    ...
    updateOverlayAddress(id, lat, lng) {
        ...
        if (addressDiv) {
            ...
            this.resolveAddress(lat, lng, (addrObj) => {
                ...
                if (addrObj.jibunAddress) {
                    html += `<span class="jibun-addr" style="font-size: 15px; color: var(--text-muted); display: block; margin-top: 2px;">(지번) ${addrObj.jibunAddress}</span>`;
                }
                if (!addrObj.roadAddress && !addrObj.jibunAddress) {
    ...
    displaySearchResults(results) {
        ...
        results.forEach(place => {
            ...
            item.addEventListener('click', () => {
                ...
                if (jibunAddr && jibunAddr !== roadAddr) {
                    tempAddrHtml += `<span class="jibun-addr" style="font-size: 15px; color: var(--text-muted); display: block; margin-top: 2px;">(지번) ${jibunAddr}</span>`;
                }
