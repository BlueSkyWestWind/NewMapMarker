...
function createOverlayContent(
  ...
  const jibun = mode === 'equipment' ? (data as EquipmentMarker).jibunAddress : '';
  const road = mode === 'equipment' ? (data as EquipmentMarker).roadAddress : (data as BatteryMarker).address;
    ...
    if (window.kakao?.maps?.services?.Geocoder) {
      ...
      geocoder.coord2Address(coord.getLng(), coord.getLat(), (result: any, status: any) => {
        if (status === window.kakao.maps.services.Status.OK && result.length > 0) {
          ...
          addressDiv.innerHTML = '<span class="road-addr">주소 조회 실패</span>';
        }
      });
    }
  }
  ...
  if (mode === 'battery') {
    ...
    teamSelectContainer.className = 'flex items-center gap-1.5 mt-2 text-[10px] text-slate-300';
    ...
    select.className = 'bg-slate-800 text-slate-100 border border-slate-700 rounded px-1 py-0.5 text-[10px] focus:outline-none cursor-pointer';
