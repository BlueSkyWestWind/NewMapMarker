...
브라우저에서 `Hydration failed ... 장비 0건 · 로딩 중` 오류가 났다. 원인은 둘이다.

1. `isLoading`을 마운트 전에도 건수 요약에 붙였다 (서버에는 조회 상태가 없다).
2. **더 큰 문제** — `mode`·`activeNav`가 zustand `persist` 대상이라
   클라이언트 첫 렌더에 이미 localStorage 값으로 복원돼 있다. 서버는 기본값으로
   HTML을 만들었으므로 레일 활성 항목·패널 내용·건수 라벨이 통째로 어긋난다.
   개편 전에는 `mode` 하나만 걸렸고 사이드바가 대부분 마운트 후 분기라 드러나지 않았는데,
   `activeNav`가 **화면 전체를 결정**하게 되면서 표면화됐다.

조치: 하이드레이션 경계를 `MapMarkerPage` **한 곳**으로 모았다.
`STORE_DEFAULT_MODE`·`STORE_DEFAULT_NAV`를 스토어에서 내보내고, 마운트 전에는 그 값으로
그린 뒤 마운트 후 실제 값으로 넘어간다. `AppShell`·`NavRail`·`WorkPanel`은 이제
스토어를 직접 읽지 않고 **props로 안전값을 받는다** — 판정이 두 곳에 생기지 않게.

검증: 프리렌더 HTML(`.next/server/app/index.html`)에서 건수 라벨이 `장비 0건`이고
활성 메뉴가 대시보드임을 확인. 남은 「로딩 중」 문자열은 지도 캔버스의 자체 placeholder다.

### 미검증 — 남은 것

| 항목 | 상태 |
| --- | --- |
| **브라우저 실물 확인** | 하이드레이션 오류는 해소 확인. 나머지 화면 렌더링·클릭 동선은 미확인 |
